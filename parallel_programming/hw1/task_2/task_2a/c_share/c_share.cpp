...
MyDeque *centralizedDeque=new MyDeque();
...
void ParRecMM(LL** z, LL **x, LL **y,int z_row,int z_col, int x_row,int x_col,int y_row,int y_col, int n, int threadId, SyncType *syncType, int workId){
	...
	// 	return;
	// }
	centralizedDeque->push_back(getWorkObject(&ParRecMM,z, x, y, z_row, z_col, x_row, x_col, y_row, y_col, n/2,threadId,s1));
	centralizedDeque->push_back(getWorkObject(&ParRecMM,z, x, y, z_row, z_col+n/2, x_row, x_col, y_row, y_col+n/2, n/2,threadId,s1));
	centralizedDeque->push_back(getWorkObject(&ParRecMM,z, x, y, z_row+n/2, z_col, x_row+n/2, x_col, y_row, y_col, n/2,threadId,s1));
	centralizedDeque->push_back(getWorkObject(&ParRecMM,z, x, y, z_row+n/2, z_col+n/2, x_row+n/2, x_col, y_row, y_col+n/2, n/2, threadId,s1));
...
void func2(LL** z, LL **x, LL **y,int z_row,int z_col, int x_row,int x_col,int y_row,int y_col, int n, int threadId, SyncType *syncType, int workId){
	...
	// 	cout<<endl;
	// }
	centralizedDeque->push_back(getWorkObject(&ParRecMM,z, x, y, z_row, z_col, x_row, x_col+n/2, y_row+n/2, y_col, n/2,threadId,syncType));
	centralizedDeque->push_back(getWorkObject(&ParRecMM,z, x, y, z_row, z_col+n/2, x_row, x_col+n/2, y_row+n/2, y_col+n/2, n/2,threadId,syncType));
	centralizedDeque->push_back(getWorkObject(&ParRecMM,z, x, y, z_row+n/2, z_col, x_row+n/2, x_col+n/2, y_row+n/2, y_col, n/2,threadId,syncType));
	centralizedDeque->push_back(getWorkObject(&ParRecMM,z, x, y, z_row+n/2, z_col+n/2, x_row+n/2, x_col+n/2, y_row+n/2, y_col+n/2, n/2, threadId,syncType));
	// allThreads[threadId]->myStack.decrementStackSyncObject(syncType);
}
...
void createParallelThreads(int num_thread){
	...
	for(int i=0;i<num_thread;++i){
		allThreads[i]->setCentralizedDeque(centralizedDeque);
	}
	// for(int i=0;i<num_thread;++i){
